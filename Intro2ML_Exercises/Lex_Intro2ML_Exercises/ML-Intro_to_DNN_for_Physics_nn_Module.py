...
class SimpleDNN(nn.Module):
    def __init__(self, N_input_features):
        ...
        self.linear1 = nn.Linear(N_input_features, 64)
        self.bn1 = nn.BatchNorm1d(64) # Include Batch normalisation      
        self.dropout1 = nn.Dropout(0.2) # Include Dropout
    ...
    def forward(self, x):
        x = self.linear1(x)
        x = self.bn1(x) # Include Batch normalisation
        x = F.relu(x)
        x = self.dropout1(x) # Include Dropout
...
def permutation_importance(model, input_data_validate_tens, targets_validate_tens):

    detach_to_binary = lambda x: np.round(model(x).detach().numpy())

    baseline_score = accuracy_score(targets_validate_tens, detach_to_binary(input_data_validate_tens))  # For a classification task
    importances = []

    for feature_idx in range(input_data_validate_tens.shape[1]):
        # Shuffle the values of the current feature
        X_val_shuffled = input_data_validate_tens.clone()
        X_val_shuffled[:, feature_idx] = X_val_shuffled[:, feature_idx][torch.randperm(input_data_validate_tens.shape[0])]

        # Recalculate the performance
        score_shuffled = accuracy_score(targets_validate_tens, detach_to_binary(X_val_shuffled))
...
# Usage example
importances = permutation_importance(model, input_data_validate_tens, targets_validate_tens)
