...
class GroupedTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        jet_token = self.jet_proj(jets)
        muon_token = self.muon_proj(muons)
        electron_token = self.electron_proj(electrons)
        met_token = self.met_proj(met)

        jet_token += self.type_emb(self.j_idx)
        muon_token += self.type_emb(self.mu_idx)
        electron_token += self.type_emb(self.el_idx)
        met_token += self.type_emb(self.met_idx)
...
plt.tight_layout()
plt.savefig("../plots/ttbar_Mass_lite.png")
plt.show()
...
# Save to file
np.savetxt(
    "../train_outputs/ttbar_mass_predictions_lite.txt", 
    results,
...
print("Saved predictions to ../data/ttbar_mass_predictions_lite.txt")
