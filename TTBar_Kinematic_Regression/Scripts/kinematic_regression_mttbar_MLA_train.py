...
# Create model with multihead latent compression
model = Transformer(
    d_model=64,
    nhead=4,
    num_layers=4,
    dropout=0.1,
    latent_dim=16  # Compression dimensiom
).to(device)
...
for epoch in range(N_epochs):
    ...
    for batch_x, batch_y, batch_m in train_loader:
        ...
        # Compute loss in normalized space against the scaled targets (batch_m is already scaled!)
        mass_loss = loss(top_m_pred_scaled, batch_m[:, 0]) + loss(antitop_m_pred_scaled, batch_m[:, 1])
