...
@dataclass
class Data_Saving:
    loss_r2_summary_plots : str = "loss_r2_summary_plots.png"
    target_feature_plots : str = "target_feature_plots.png"
    target_dimension_scatter_plots : str = "target_dimension_scatter_plots.png"
    invariant_mass_plots : str = "invariant_mass_plots.png"
    invariant_mass_data : str = "invariant_mass_data.txt"
...
for epoch in range(control_panel.train_config.num_epochs):
    ...
    for batch_x, batch_y, batch_m in train_loader:
        ...
        top_m_pred_scaled = (top_m_pred - scaler_M_mean[0]) / scaler_M_scale[0]
        antitop_m_pred_scaled = (antitop_m_pred - scaler_M_mean[1]) / scaler_M_scale[1]
        ...
        individual_mass_loss = loss(top_m_pred_scaled, batch_m[:, 0]) + loss(antitop_m_pred_scaled, batch_m[:, 1])

        system_mass_loss = loss(m_ttbar_pred / 500.0, m_ttbar_true / 500.0)
...
kl_mass, std_mass, ci_mass, _ = bootstrap_kl_divergence(
    M_pred, M_true, n_bootstrap=1000, bins=100
)
