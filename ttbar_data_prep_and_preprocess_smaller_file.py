...
with h5py.File("train_inputs/smaller_ttbar_train.h5", "w") as f:
    f.create_dataset("X", data=X_train_scaled)
    f.create_dataset("Y", data=Y_train_scaled)

with h5py.File("train_inputs/smaller_ttbar_val.h5", "w") as f:
    f.create_dataset("X", data=X_val_scaled)
    f.create_dataset("Y", data=Y_val_scaled)

with h5py.File("train_inputs/smaller_ttbar_test.h5", "w") as f:
    f.create_dataset("X", data=X_test_scaled)
    f.create_dataset("Y", data=Y_test_scaled)

with h5py.File("train_inputs/smaller_scaler_info.h5", "w") as f:
