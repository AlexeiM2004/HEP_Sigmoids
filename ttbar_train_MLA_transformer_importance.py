...
class Transformer(nn.Module):
    def __init__(self, d_model=64, nhead=4, num_layers=8, dropout=0.1, latent_dim=16):
        super().__init__()
        
        # ========== MLA-STYLE LATENT COMPRESSION ==========
        # Instead of projecting directly to d_model, we compress to latent_dim first
        # This forces the model to learn a more efficient representation
...
axes[1,0].hist(Y_test_geV, bins=50, color='red', alpha=0.5, edgecolor='black')
axes[1,0].set_xlabel("Predicted ttbar mass and true ttbar mass (GeV)")
...
plt.tight_layout()
plt.savefig("ttbar_analysis_larger_4head_8layer_8workers.png")
plt.show()
...
# Save to file
np.savetxt(
    "ttbar_analysis_larger_4head_8layer_8workers.txt", 
    results,
...
print("Saved predictions to ttbar_analysis_larger_4head_8layer_8workers.txt")
print("Saved analysis plot to ttbar_analysis_larger_4head_8layer_8workers.png")