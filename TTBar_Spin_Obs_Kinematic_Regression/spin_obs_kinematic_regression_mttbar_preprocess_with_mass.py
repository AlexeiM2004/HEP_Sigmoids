...
# Calculate mass to use in loss loop
...
### ------------------------------ Extract Lepton Features from Reco ------------------------------ ###

leptonP_pt, leptonP_phi, leptonP_eta, leptonP_m = [tree_awk[f"parton_lepton_plus_{s}"] for s in ["pt", "phi", "eta", "m"]]
leptonM_pt, leptonM_phi, leptonM_eta, leptonM_m = [tree_awk[f"parton_lepton_minus_{s}"] for s in ["pt", "phi", "eta", "m"]]

lepP_px, lepP_py, lepP_pz, lepP_E = momentum_energy_calculator(leptonP_pt, leptonP_phi, leptonP_eta, leptonP_m)
lepM_px, lepM_py, lepM_pz, lepM_E = momentum_energy_calculator(leptonM_pt, leptonM_phi, leptonM_eta, leptonM_m)

lepton_features = np.column_stack([
    ak.to_numpy(lepP_px), ak.to_numpy(lepP_py), ak.to_numpy(lepP_pz), ak.to_numpy(lepP_E),
    ak.to_numpy(lepM_px), ak.to_numpy(lepM_py), ak.to_numpy(lepM_pz), ak.to_numpy(lepM_E)
])
print(f"Lepton features shape: {lepton_features.shape}")

### ------------------------------ Clean Memory ------------------------------ ###

del X_features, tree, file_ttbar, Target_features, top_pt, top_phi, top_eta, top_m, antitop_pt, antitop_phi, antitop_eta, antitop_m
...
X_train, X_temp, Y_train, Y_temp, M_train, M_temp, L_train, L_temp = train_test_split(
    X, target, mass, lepton_features, test_size=0.2, random_state=67)

# Second split: 10% validation, 10% test
X_val, X_test, Y_val, Y_test, M_val, M_test, L_val, L_test = train_test_split(
    X_temp, Y_temp, M_temp, L_temp, test_size=0.5, random_state=67)
